...
왼쪽 부분이 네이티브 언어로 만들어진 부분입니다. 네이티브 앱은 OEM 위젯이라 하는 플랫폼이 지원하는 위젯들과 소통하여 어떤 위젯을 화면에 표시할지 정하고, 캔버스라는 화면 표시기를 통해 화면에 노출하게 됩니다. 그리고 해당 위젯은 이벤트와 연결되어 이벤트를 결정짓기도 합니다. 
...
하지만 결국 자바스크립트에서 브릿지를 거쳐야하는 이 메커니즘에는 성능 이슈를 계속 야기해왔고 구글은 이를 해결하기 위한 방법을 모색하기 시작했습니다. Flutter는 브릿지 부분을 빼버리고 아예 네이티브 앱으로 컴파일하여 플랫폼으로 넘겨주는 방식을 채택했습니다. 

이게 무슨말인가하면, Flutter는 일종의 프레임워크이며 Flutter 자체적으로 위젯을 제공하여 Dart로 작성된 앱을 컴파일 하여 네이티브 앱으로 렌더링해 앱을 만드는 방식이 됩니다. 구글은 이러한 방식을 꾀함으로써 크로스플랫폼 앱이 가지는 성능적인 이슈들(브릿지를 거치는 방식)을 해결했습니다.
...
플러터 엔진의 코드를 살펴보면 [AttemptVMLaunchWithCurrentSettings](https://github.com/flutter/engine/blob/923687f0e7ff0cddcbd05c98fd3ca12db9c45ac8/shell/platform/fuchsia/flutter/component.cc#L500)라는 함수에서 어플리케이션 실행 시에 isolate_snapshot_data와 vm_snapshot_data를 load하고 있는 것을 볼 수 있습니다.
...
## 6. 앞으로 실무에서 사용할 예정인가?
위에서 소개드린바와 같이 이번 프로젝트를 통해 플러터의 성능, 머티리얼 디자인(Material Design) 가이드, CI/CD, 애플리케이션 아키텍쳐(bloc 패턴) 모두 만족할만한 결과를 얻었습니다. 하지만 장단점을 논하기엔 짧은 기간의 개발이라 조심스럽고 섣부른 판단일수도있습니다만, 플러터 자체적으로는 실무에 적용하기에 충분하지만, 플러터가 제공하는 라이브러리가 현재로서는 아직 부족한 감이 없지않나 싶습니다.
...
하지만 플러터는 아직 성장단계이며 오픈소스 라이브러리의 이슈는 플러터의 성장에따라 이슈가 적어지리라 생각됩니다. 2018년도에 배우지 말아야할 언어 1위로 선정되기도 했지만 구글과 관련 개발자들의 각고의 노력으로 2019년도에는 Job market 부분을 제외하고선 모두 하위권으로 내려간 상태입니다. 
